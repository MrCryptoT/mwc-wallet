...
use crate::libwallet::{
    ErrorKind,  NodeClient, Slate, WalletInst,
    WalletLCProvider,
};

pub fn process_receiver_initiated_slate<'a, L, C, K>(
    wallet: Arc<Mutex<Box<dyn WalletInst<'a, L, C, K>>>>,
    slate: &mut Slate, address: Option<String> ) -> Result<(), Error>
    where
...
//		.ok_or(ErrorKind::DoesNotAcceptInvoices)?;
    ...
    *slate = invoice_tx(wallet.clone(),
                        Some(active_account.to_string()), slate,
                        address.clone(),
                        10, 500, 1,
                        false, None)?;
...
{
    let s = receive_tx(wallet, address, slate,
...
pub fn finalize_slate<'a, L, C, K>(
    wallet: Arc<Mutex<Box<dyn WalletInst<'a, L, C, K>>>>,
    slate: &mut Slate,
    tx_proof: Option<&mut TxProof>) -> Result<(), Error>
    where
        L: WalletLCProvider<'a, C, K>,
        C: NodeClient + 'a,
        K: Keychain + 'a,
{
    ...
    let should_post = finalize_tx( wallet.clone(), slate, tx_proof).map_err(|_| ErrorKind::GrinWalletFinalizeError)?;
        ...
        K: Keychain + 'a,
{
    ...
    let should_post = finalize_invoice_tx(wallet.clone(), slate).map_err(|_| ErrorKind::GrinWalletFinalizeError)?;
